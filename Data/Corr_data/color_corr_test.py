...
if '1' in option_bin_set:
	...
	scatter = axes[length_data_list[j]].scatter(data['uncorr_integral'], data['goes_max_proton'], c=duration, cmap=color_map_option, zorder=5)
	...
	axes[length_data_list[j]].set_ylabel(f'Max Proton Flux [pfu]', fontname="Arial", fontsize = 16)
	axes[length_data_list[j]].set_xlabel(r'Uncorrected Type III Radio Burst Integral Intensity [dB]', fontname="Arial", fontsize = 16)

	applyPlotStyle()
	plt.savefig('color_uncorrected.png', format='png', dpi=300) # 900
	plt.show()
	...
	scatter = axes[length_data_list[j]].scatter(data['integral'], data['goes_max_proton'], c=duration, cmap=color_map_option, zorder=5)
	plt.colorbar(scatter,fraction=0.046, pad=0.04)
	...
	# axes[length_data_list[j]].set_yticks([10**-3, 10**-2, 10**-1, 10**0, 10**1, 10**2, 10**3])
	axes[length_data_list[j]].set_ylabel(f'Max Proton Flux [pfu]', fontname="Arial", fontsize = 16)
	axes[length_data_list[j]].set_xlabel(r'Corrected Type III Radio Burst Integral Intensity [dB]', fontname="Arial", fontsize = 16)

	applyPlotStyle()
	plt.savefig('color_corrected.png', format='png', dpi=300) # 900
