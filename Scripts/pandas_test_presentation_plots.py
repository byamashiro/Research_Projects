...
def applyPlotStyle():
	...
	if '1' in option_bin_set:
		...
		axes[length_data_list[j]].axvline(proton_df[f'{high_bin_proton}'].loc[f'{event_obj_start_str_date}':f'{event_obj_end_str_date}'].idxmax(), linewidth=3, zorder=1, color='green', linestyle=':') #, label='Max >100 MeV') # (proton_df.P6W_UNCOR_FLUX.max()) # changed maximum flux to be within time interval specified
	
	if '2' in option_bin_set:
		if len(rb_event_df) != 0:
			axes[length_data_list[j]].axvline(rb_event_df['t3_max_time'].loc[rb_event_df['t3_duration'].idxmax()], linewidth=3, zorder=1, color='green', linestyle=':') # , label='Max T3 Intensity') # (proton_df.P6W_UNCOR_FLUX.max()) # changed maximum flux to be within time interval specified
	...
	# borderaxespad=0)# bbox_to_anchor=(1, 0.5)) # bbox_to_anchor=(1.02,1.0)
...
if '1' in option_bin_set:
	...
	if goes_corrected_option == 'yes':
		...
		if proton_event_option == 'smooth':
			axes[length_data_list[j]].plot(proton_smooth_df[f'{butter_filter}'].loc[f'{event_obj_start_str_date}':f'{event_obj_end_str_date}'], color='purple', linewidth=2, label=f'Butterworth Filter', zorder=5)
		...
		axes[length_data_list[j]].set_ylabel(f'GOES-{satellite_no} Proton Flux [pfu]', fontname="Arial", fontsize = 16)
		axes[length_data_list[j]].legend(loc='upper right', ncol=1,fontsize=12)
	...
	for i in range(len(proton_event_df)):
		if s_dtime <= (proton_event_df['start_time'][i] and proton_event_df['end_time'][i]) <= e_dtime:
			axes[length_data_list[j]].axvspan(proton_event_df['start_time'][i], proton_event_df['end_time'][i], color='moccasin', alpha=0.5)
			axes[length_data_list[j]].axvline(proton_event_df['start_time'][i], linewidth=1, zorder=2, color='darkcyan', linestyle='-') #  xmin=0, xmax=1
			axes[length_data_list[j]].axvline(proton_event_df['end_time'][i], linewidth=1, zorder=2, color='darkcyan', linestyle='-') #  xmin=0, xmax=1
...
if '2' in option_bin_set:
	...
	for frequency in freq_list:
		...
		axes[length_data_list[j]].plot(rb_data_df_interp[frequency].loc[f'{event_obj_start_str_date}':f'{event_obj_end_str_date}'], 'o', mfc='none', color='purple', zorder=4)
		axes[length_data_list[j]].plot(rb_data[frequency].loc[f'{event_obj_start_str_date}':f'{event_obj_end_str_date}'], 'o', mfc='none', color=color_choice, zorder=5) # working plotting feature, without stem
	...
	for rbevent_no_interp in range(len(rb_list_event_interp)):
		...
		for rbpoint_interp in rb_list_event_interp[rbevent_no_interp]:
			axes[length_data_list[j]].vlines(x=rbpoint_interp, ymin=t3_threshold_interp, ymax=rb_data_df_interp['avg'].loc[rbpoint_interp], linewidth=1, color='teal', zorder=2)
	...
	for i in range(len(rb_event_df)):
		if s_dtime <= (rb_event_df['start_time'][i] and rb_event_df['end_time'][i]) <= e_dtime:
			axes[length_data_list[j]].axvspan(rb_event_df_interp['start_time'][i], rb_event_df_interp['end_time'][i], color='moccasin', alpha=0.5)
			axes[length_data_list[j]].axvline(rb_event_df_interp['start_time'][i], linewidth=1, zorder=2, color='darkcyan', linestyle='-') #  xmin=0, xmax=1
			axes[length_data_list[j]].axvline(rb_event_df_interp['end_time'][i], linewidth=1, zorder=2, color='darkcyan', linestyle='-') #  xmin=0, xmax=1
	...
	axes[length_data_list[j]].set_ylabel('Wind Frequency Averaged\nType III Radio Burst Intensity [dB]', fontname="Arial", fontsize = 16)
...
plt.savefig(f'presentation_plots/{option_bin_set}_plot.png', format='png', dpi=900)
