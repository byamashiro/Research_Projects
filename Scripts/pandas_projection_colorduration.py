...
# color_cm=iter(cm.viridis(np.linspace(0,int(data['connectivity'].min()), int(data['connectivity'].max())  ) ) )
...
full_df = pd.DataFrame(np.NaN, columns=y, index=x)


pair_data = []
for i in range(len(data)):
	pair_data.append([data['flare_long'][i],data['flare_lat'][i]])

for i in range(len(full_df.index)):
	for j in range(len(full_df.columns)):
		# print([full_df.index[i],full_df.columns[j]])
		print(full_df.index[i], " ", full_df.columns[j])
		if [full_df.index[i],full_df.columns[j]] in pair_data:
			print("i: ",i," j: ",j)
			full_df.iloc[i, j] = 1
'''

			# full_df.set_value(full_df.index[i], full_df.columns[j], data['connectivity'].loc[data['flare_long']==full_df.index[i]].loc[data['flare_lat']==full_df.columns[j]])
			# full_df[] = data['connectivity'].loc[data['flare_long']==full_df.index[i]].loc[data['flare_lat']==full_df.columns[j]]
'''
for i in range(len(data)):
	full_df.set_value(data['flare_long'][i], data['flare_lat'][i], data['connectivity'][i])
'''
x = data['flare_long']
y = data['flare_lat']
z = data['connectivity']
...
scatter = ax.scatter(data['flare_long'], data['flare_lat'], c=data['duration'], cmap='viridis', transform=ccrs.Geodetic(), zorder=3)
