...
detection_threshold = 0.5 # 0.25
detection_threshold_str = '0d50'
energy_channel = 50
	...
	for event_day in (full_year):
		...
		if plot_check == True:
			...
			for sat in ['13','15']:
				...
				plt.plot(proton_df[f'{energy_header}'].loc[f'{event_day[0]}':f'{event_day[-1]}'], label = f'GOES-{sat} >50 MeV', color=col_def)
			...
			if ''.join(event_day[0]) in list(year_set_13.difference(year_set_15)):
				...
				plt.title(f'Proton Event Detector [GOES-13/15 CONFIRMED]\n[{event_day[0]} -- {event_day[-1]}] (Threshold : {detection_threshold} -- {energy_channel} MeV)', fontname="Arial", fontsize = 14) #, y=1.04,
			...
			plt.savefig(f'detected_events/50mev/{detection_threshold_str}pfu_{energy_channel}mev_{event_day[0]}.png', format='png', dpi=900)
			#sys.exit(0)
